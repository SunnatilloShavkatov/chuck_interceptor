      ...
      children: [
        ...
        if (queryParams.isNotEmpty)
          buildCardSection(
            ...
            children: queryParams.entries.map((e) => getListRow('${e.key}:', e.value.toString(), copyable: true)).toList(),
          ),
        buildCardSection(
        ...
        if (request.formDataFiles?.isNotEmpty ?? false)
          buildCardSection(
            ...
            children: request.formDataFiles!.map((f) => getListRow(f.fileName ?? 'file', '${f.contentType} • ${f.length} B')).toList(),
          ),
        buildCardSection(
          title: 'Request Body',
