...
class _MyAppState extends State<MyApp> {
  ...
  Widget _getTextWidget(String text) => Text(text, style: const TextStyle(fontSize: 14), textAlign: TextAlign.center);

  Future<void> _runDioRequests() async {
    final Map<String, dynamic> body = <String, dynamic>{'title': 'foo', 'body': 'bar', 'userId': '1'};
    ...
    _dio.get<void>('https://jsonplaceholder.typicode.com/posts', queryParameters: <String, dynamic>{'test': 1});
  ...
  void _runHttpHttpClientRequests() {
    final Map<String, dynamic> body = <String, dynamic>{'title': 'foo', 'body': 'bar', 'userId': '1'};
    _httpClient.getUrl(Uri.parse('https://jsonplaceholder.typicode.com/posts')).interceptWithChuck(_chuck);
    ...
    _httpClient.getUrl(Uri.parse('https://jsonplaceholder.typicode.com/test/test/')).interceptWithChuck(_chuck);
  ...
  Future<void> _runGenericClientRequests() async {
    ...
    _chuck.logHttpCall(
      ...
      responseHeaders: <String, String>{'content-type': response.headers.contentType?.toString() ?? ''},
    ...
    try {
      ...
      _chuck.logResponse(
        ...
        headers: <String, String>{'content-type': postResponse.headers.contentType?.toString() ?? ''},
